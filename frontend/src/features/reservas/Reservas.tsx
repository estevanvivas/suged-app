...
const calcularEstadoVisual = (reserva: any) => {
  ...
  const fechaLocal = new Date(hoy.getTime() - (hoy.getTimezoneOffset() * 60000))
    .toISOString()
    .split('T')[0];
  const horaActualStr = `${hoy.getHours().toString().padStart(2, '0')}:${hoy
    .getMinutes()
    .toString()
    .padStart(2, '0')}:00`;
...
export default function Reservas() {
  ...
  const [datosPerfilFaltantes, setDatosPerfilFaltantes] = useState({
    documento: '',
    codigo: '',
    carrera: ''
  });
  ...
  const manejarCambioEstado = async (reserva: any, nuevoEstado: string) => {
    ...
    try {
      ...
      toast.success(`Reserva ${nuevoEstado.replace('_', ' ').toLowerCase()} con éxito`, {
        id: toastId
      });
  ...
  const reservasPendientesAdmin = reservasAdmin.filter((res) => res.estado === 'PENDIENTE');
  const reservasAprobadasAdmin = reservasAdmin.filter((res) => res.estado === 'APROBADA');
      ...
      <div className="bg-white p-4 md:p-8 rounded-xl shadow-sm border border-slate-200 min-h-[80vh]">
        <div className="mb-8 border-b border-slate-100 pb-4">
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Bandeja de pendientes y reservas confirmadas.
          </p>
        </div>
        ...
        <div className="space-y-10">
          <div>
            <h2 className="text-lg font-black text-[#1A1A1A] mb-4 flex items-center gap-2">
              <Ticket className="text-[#FFCC29]" /> Bandeja de pendientes
            </h2>

            {reservasPendientesAdmin.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {reservasPendientesAdmin.map((res) => (
                  <div
                    key={res.id}
                    className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4"
                  >
                    <div className="flex-1">
                      <h3 className="font-bold text-[#1A1A1A] text-base">
                        {res.usuarios?.nombre_completo}
                      </h3>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs text-slate-500 mt-3">
                        ...
                        <span className="flex items-center gap-1">
                          <Clock size={12} /> {res.hora_inicio.slice(0, 5)} -{' '}
          ...
          <div>
            <h2 className="text-lg font-black text-[#1A1A1A] mb-4 flex items-center gap-2 border-t border-slate-100 pt-8">
              <Sparkles className="text-[#FFCC29]" /> Reservas confirmadas
            </h2>

            {reservasAprobadasAdmin.length === 0 ? (
              <div className="text-center py-16 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                <CheckCircle2 className="mx-auto h-12 w-12 text-slate-400 mb-3" />
                <h3 className="text-lg font-medium text-[#1A1A1A]">No hay reservas confirmadas</h3>
                <p className="text-sm text-slate-500 mt-1">
                  Las reservas aprobadas aparecerán aquí con su QR.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {reservasAprobadasAdmin.map((res) => (
                  <div
                    key={res.id}
                    className="bg-[#1A1A1A] text-white rounded-2xl overflow-hidden shadow-lg border border-white/10 flex flex-col relative"
                  >
                    ...
                    <div className="p-5 border-b border-white/5 relative z-10 flex justify-between items-start">
                      <div className="pr-4">
                        <h3 className="font-black text-xl leading-tight mb-2 text-white">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <span className="flex items-center gap-1.5 text-xs font-medium text-slate-300">
                            <Clock size={14} className="text-[#FFCC29]" /> {res.hora_inicio.slice(0, 5)} -{' '}
                            {res.hora_fin.slice(0, 5)}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="p-5 relative z-10 bg-white/5 flex flex-col items-center justify-center min-h-[180px]">
                    ...
                    <div className="p-4 border-t border-white/5">
                      <button
                        onClick={() => manejarCambioEstado(res, 'CANCELADA')}
                        className="w-full text-xs bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white px-3 py-2.5 rounded-lg font-bold transition-colors"
                      >
                        Cancelar reserva
                      </button>
                    </div>
          ...
          <div className="animate-in fade-in duration-500">
            <div className="max-w-2xl mx-auto mb-12 md:mb-16 mt-4">
              <div className="flex items-center justify-between relative px-2 md:px-8">
                ...
                {[
                  ...
                  <div
                    key={s.num}
                    className="relative z-10 flex flex-col items-center gap-2 md:gap-3 bg-white px-2"
                  >
                    <button
                      ...
                      className={`w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center transition-all duration-500 ${
                        paso >= s.num
                          ? 'bg-[#1A1A1A] text-[#FFCC29] shadow-lg shadow-[#1A1A1A]/20 scale-110'
                    ...
                    <span
                      className={`text-[10px] md:text-xs font-black uppercase tracking-widest transition-colors duration-500 ${
                        paso >= s.num ? 'text-[#1A1A1A]' : 'text-slate-300'
                      }`}
                    >
                      {s.label}
                    </span>
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 p-6 w-full">
                      <div className="flex justify-between items-end">
                        <div>
                          <h3 className="font-black text-white text-2xl mb-1 group-hover:text-[#FFCC29] transition-colors">
                            {esc.nombre}
                          </h3>
            ...
            {paso === 3 && fechaSeleccionada && escenarioSeleccionado && (
              <div className="animate-in slide-in-from-right-8 duration-500">
                ...
                {cargandoHoras ? (
                  <p className="text-center text-slate-500 py-12 animate-pulse font-medium">
                    Buscando espacios libres...
                  </p>
                ) : bloquesLibres.length === 0 ? (
                  <p className="text-center text-red-500 py-12 font-bold">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {bloquesLibres.map((bloque, i) => (
                      <button
                        ...
                        className={`p-3 rounded-xl border-2 font-bold text-sm flex justify-center items-center gap-1 transition-all ${
                          bloqueSeleccionado?.hora_inicio === bloque.hora_inicio
                            ? 'border-[#1A1A1A] bg-[#1A1A1A] text-[#FFCC29] shadow-lg scale-105'
                            : 'border-slate-200 bg-white hover:border-[#FFCC29] text-slate-600'
                        }`}
                      >
                        {bloque.etiqueta}
                      </button>
                    ...
                    <div className="bg-[#1A1A1A] rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-2xl border border-[#FFCC29]/20">
                      ...
                      <div className="relative z-10 flex flex-col md:flex-row gap-8">
                        <div className="flex-1 md:pr-6">
                          ...
                          <p className="text-3xl md:text-4xl font-black text-white leading-tight mb-8">
                          ...
                          {leFaltanDatos ? (
                            <form onSubmit={manejarGuardarPerfilYReservar} className="space-y-4 animate-in fade-in">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setDatosPerfilFaltantes({
                                    ...datosPerfilFaltantes,
          ...
          <div className="animate-in fade-in space-y-10">
            {reservasVigentes.length === 0 && reservasPasadas.length === 0 && (
              <div className="text-center py-16 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Cuando agendes un espacio, aparecerá aquí.
                </p>
                <button
                  onClick={() => setVistaActiva('NUEVA')}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {reservasVigentes.map((res) => (
                    <div
                      key={res.id}
                      className="bg-[#1A1A1A] text-white rounded-2xl overflow-hidden shadow-lg border border-white/10 flex flex-col relative group"
                    >
                      <div className="absolute top-0 right-[25%] w-px h-full bg-white/10 border-dashed border-l border-white/20"></div>
                      <div className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-white rounded-full opacity-10"></div>
                      <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-white rounded-full opacity-10"></div>
                  ...
                  <div className="grid grid-cols-1 divide-y divide-slate-200">
                    {reservasPasadas.map((res) => {
                      ...
                      return (
                        <div
                          key={res.id}
                          className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-100 transition-colors"
                        >
